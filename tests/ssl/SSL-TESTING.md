...
docker compose -f docker-compose.yml -f compose/ssl.yml --profile ssl logs certbot
...
docker compose -f docker-compose.yml -f compose/ssl.yml --profile ssl down -v
...
```bash
docker exec sihsalus-gateway ls -la /etc/letsencrypt/live/yourdomain.com/
```

**Verify nginx configuration:**
```bash
docker exec sihsalus-gateway nginx -t
```
...
**Manual renewal test:**
```bash
docker compose -f docker-compose.yml -f compose/ssl.yml --profile ssl run --rm certbot renew --dry-run
```

**Force renewal:**
```bash
docker compose -f docker-compose.yml -f compose/ssl.yml --profile ssl run --rm certbot renew --force-renewal
```

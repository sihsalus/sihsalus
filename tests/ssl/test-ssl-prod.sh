...
if [ "$EXTERNAL_MODE" != "external" ]; then
    ...
    if docker compose -f docker-compose.yml -f compose/ssl.yml --profile ssl ps 2>/dev/null | grep -q "gateway.*Up"; then
        print_success "Gateway container is running"
    else
        print_failure "Gateway container is not running"
    fi

    if docker compose -f docker-compose.yml -f compose/ssl.yml --profile ssl ps 2>/dev/null | grep -q "backend.*Up"; then
        print_success "Backend container is running"
    else
        print_failure "Backend container is not running"
    fi

    if docker compose -f docker-compose.yml -f compose/ssl.yml --profile ssl ps 2>/dev/null | grep -q "frontend.*Up"; then
    ...
    if docker exec sihsalus-gateway test -f /etc/letsencrypt/live/$DOMAIN/fullchain.pem 2>/dev/null; then
        print_success "Certificate file exists: fullchain.pem"
    else
        print_failure "Certificate file missing: fullchain.pem"
    fi

    if docker exec sihsalus-gateway test -f /etc/letsencrypt/live/$DOMAIN/privkey.pem 2>/dev/null; then
    ...
    CERTBOT_STATUS=$(docker compose -f docker-compose.yml -f compose/ssl.yml --profile ssl ps -a certbot 2>/dev/null | grep certbot 2>/dev/null)
    ...
    if docker exec sihsalus-gateway ps aux 2>/dev/null | grep -q "watch-certs.sh"; then
        print_success "Certificate watcher process is running"
        WATCHER_PID=$(docker exec sihsalus-gateway ps aux 2>/dev/null | grep "watch-certs.sh" | grep -v grep | awk '{print $1}')
    ...
    if docker compose -f docker-compose.yml -f compose/ssl.yml --profile ssl ps gateway 2>/dev/null | grep -q "443"; then
        print_success "Nginx listening on port 443 (HTTPS)"
    else
        print_failure "Nginx not listening on port 443"
    fi

    if docker compose -f docker-compose.yml -f compose/ssl.yml --profile ssl ps gateway 2>/dev/null | grep -q "80"; then

...
if docker exec sihsalus-gateway test -f /etc/letsencrypt/live/localhost/fullchain.pem; then
    print_success "Certificate file exists: fullchain.pem"
else
    print_failure "Certificate file missing: fullchain.pem"
fi

if docker exec sihsalus-gateway test -f /etc/letsencrypt/live/localhost/privkey.pem; then
...
if docker compose -f docker-compose.yml -f compose/ssl.yml --profile ssl ps gateway 2>/dev/null | grep -q "443->443"; then
    print_success "Nginx listening on port 443 (HTTPS)"
else
    print_failure "Nginx not listening on port 443"
fi

if docker compose -f docker-compose.yml -f compose/ssl.yml --profile ssl ps gateway 2>/dev/null | grep -q "80->80"; then

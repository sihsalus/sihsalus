...
trap 'if [ "${KEEP_TEST_ROOT:-false}" = true ]; then echo "test files retained at $TEST_ROOT" >&2; else rm -rf "$TEST_ROOT"; fi' EXIT
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    --connect-timeout | --max-time | --header | --dump-header | --output | --write-out | --cacert | --pinnedpubkey)
      option="$1"
      value="$2"
      shift 2
      case "$option" in
        ...
        --pinnedpubkey) printf '%s\n' "$value" >>"$FAKE_STATE_DIR/pins" ;;
        --cacert) printf '%s\n' "$value" >>"$FAKE_STATE_DIR/ca-paths" ;;
      esac
      ;;
    --insecure)
      printf '%s\n' insecure >>"$FAKE_STATE_DIR/tls-options"
      shift
      ;;
...
if [[ "$url" == *'/openmrs/spa/build-info.json?'* ]]; then
...

  {
    ...
    if [ -n "$digest" ]; then
      printf 'X-SIHSALUS-Frontend-Digest: %s\r\n' "$digest"
    fi
    printf '\r\n'
  } >"$header_file"
  printf '{"gitSha":"%s"}\n' "$sha" >"$output_file"
...
run_verifier() {
  ...
  rm -f "$FAKE_STATE_DIR/requests" "$FAKE_STATE_DIR/build-requests" \
    "$FAKE_STATE_DIR/pins" "$FAKE_STATE_DIR/ca-paths" "$FAKE_STATE_DIR/tls-options"
  PATH="$FAKE_BIN:$PATH" \
    ...
    bash "$VERIFIER" https://example.test "$TARGET_SHA" "$TARGET_DIGEST" "$TARGET_NODE_ID" TEST >"$output" 2>&1
}
...
grep -Fq "observed source digest(s): $TARGET_DIGEST" "$consistent_output"
grep -Fq "observed node identity/identities: $TARGET_NODE_ID" "$consistent_output"
grep -Fq 'all 4 observations match' "$consistent_output"
[ "$(wc -l <"$FAKE_STATE_DIR/requests" | tr -d ' ')" -eq 16 ]
[ "$(cat "$FAKE_STATE_DIR/build-requests")" -eq 4 ]
[ ! -e "$FAKE_STATE_DIR/tls-options" ]
[ ! -e "$FAKE_STATE_DIR/pins" ]
...
assert_fails mismatch-digest
grep -Fq "serves source digest $BAD_DIGEST, expected $TARGET_DIGEST" "$TEST_ROOT/mismatch-digest.log"

assert_fails split-digest
grep -Fq "observed source digest(s): $TARGET_DIGEST $BAD_DIGEST" "$TEST_ROOT/split-digest.log"
grep -Fq 'exposed multiple or indeterminate frontend source digests' "$TEST_ROOT/split-digest.log"

missing_digest_output="$TEST_ROOT/no-digest.log"
set +e
...
grep -Fq 'all 4 observations match' "$transient_output"
...
if PATH="$FAKE_BIN:$PATH" \
  ...
  bash "$VERIFIER" https://example.test "$TARGET_SHA" "$TARGET_DIGEST" "$TARGET_NODE_ID" TEST >"$invalid_count_output" 2>&1; then
  echo 'verifier accepted a single sample' >&2
  exit 1
fi
grep -Fq 'must be an integer of at least 2' "$invalid_count_output"

insecure_without_pin_output="$TEST_ROOT/insecure-without-pin.log"
if PATH="$FAKE_BIN:$PATH" \
  EXTERNAL_VERIFY_TLS_INSECURE=true \
  bash "$VERIFIER" https://example.test "$TARGET_SHA" "$TARGET_DIGEST" "$TARGET_NODE_ID" TEST >"$insecure_without_pin_output" 2>&1; then
  echo 'verifier accepted unauthenticated insecure TLS' >&2
  exit 1
fi
grep -Fq 'only when a protected SPKI pin is also verified' "$insecure_without_pin_output"

PIN='sha256//AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA='
pinned_output="$TEST_ROOT/pinned.log"
EXTERNAL_VERIFY_TLS_INSECURE=true \
  EXTERNAL_VERIFY_TLS_PINNED_PUBLIC_KEY="$PIN" \
  run_verifier consistent "$pinned_output"
grep -Fxq "$PIN" "$FAKE_STATE_DIR/pins"
grep -Fxq insecure "$FAKE_STATE_DIR/tls-options"

CA_CERT="$TEST_ROOT/private-ca.pem"
printf '%s\n' 'test CA fixture' >"$CA_CERT"
ca_output="$TEST_ROOT/ca.log"
EXTERNAL_VERIFY_TLS_CA_CERT_PATH="$CA_CERT" \
  run_verifier consistent "$ca_output"
grep -Fxq "$CA_CERT" "$FAKE_STATE_DIR/ca-paths"
[ ! -e "$FAKE_STATE_DIR/tls-options" ]

echo '[OK] TLS-authenticated, release-identity-bound external frontend verification'

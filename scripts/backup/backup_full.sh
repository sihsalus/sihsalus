...
mapfile -t LOG_FILES < <(find "$FULL_BACKUP_DIR" -maxdepth 1 -type f -name 'fullBackup_log.txt*' -printf '%T@ %p\n' | sort -rn | cut -d' ' -f2-)
...
echo "[OK] Backup completo realizado en '$FULL_BACKUP_DIR/$BACKUP_NAME.tar.gz.enc'"

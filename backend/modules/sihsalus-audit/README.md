...
- Free-text-compatible `message` and `componentStack` values remain accepted within the 64 KiB request bound but are discarded without an individual length rejection. Persistence is limited to an enumerated frontend `appName`, enumerated `outcome`, boolean `offline`, and UUID `locationUuid`; unknown machine values are also discarded so they cannot become a durable PHI/secret channel. Review applies the same canonicalization to legacy rows and omits malformed metadata without changing the stored evidence.

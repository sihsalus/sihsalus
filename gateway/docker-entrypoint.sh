...
if [ -z "${CERT_WEB_DOMAIN_COMMON_NAME}" ] && [ -n "${CERT_WEB_DOMAINS}" ]; then
    CERT_WEB_DOMAIN_COMMON_NAME=${CERT_WEB_DOMAINS%%,*}
...
if [ -n "${CERT_WEB_DOMAIN_COMMON_NAME}" ]; then
    ...
    cp /etc/nginx/conf-templates/https.conf.template /etc/nginx/templates/default.conf.template
    ...
    cp /etc/nginx/conf-templates/http.conf.template /etc/nginx/templates/default.conf.template
fi
